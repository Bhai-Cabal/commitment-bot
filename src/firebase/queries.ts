...
export async function getRanking(
  ...
  try {
    ...
    let ranking = "🏆 Bhai Cabal Fitness Hall of Fame 🏆\n\n";
    ...
    return (
      ranking ||
      "No workouts logged yet? Time to get moving, Bhai Cabal athletes! 💪🏋️‍♀️"
    );
  } catch (error) {
...
export async function getShippingRanking(
  ...
  try {
    ...
    let ranking = "🚢 Bhai Shipping Hall of Fame 🚢\n\n";
    usersSnapshot.docs.forEach((doc, index) => {
      const data = doc.data();
      const emoji = getPlacementEmoji(index);
    ...
    return (
      ranking ||
      "No ships launched yet? Time to start coding and shipping, Bhai Cabal developers! 👩‍💻🚀"
    );
  } catch (error) {
...
export async function getMindfulnessRanking(
  ...
  try {
    ...
    let ranking = "🧘 Bhai Cabal Mindfulness Hall of Zen 🧘\n\n";

    if (usersSnapshot.empty) {
      return "No zen moments logged yet? Time to start finding your inner peace, Bhai Cabal mindfulness seekers! 🧘‍♂️✨";
    }
